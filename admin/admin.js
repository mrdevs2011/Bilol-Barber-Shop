...
// ---------------------------------------------------------------------------
// Realtime: yangi bron kelishi, holat o'zgarishi yoki o'chirilishi bilanoq
...
async function renderNbSlots() {
  ...
  nbSlots.innerHTML = slots.map(t => {
    ...
    return `<button type="button" class="nb-slot${selected ? ' selected' : ''}" data-time="${t}" ${disabled ? 'disabled' : ''}>${t}</button>`;
  }).join('');
